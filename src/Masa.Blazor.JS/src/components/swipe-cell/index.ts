...
export interface SwipeCellOptions {
  /** 滑动阈值，超过此值才会打开/关闭 */
  threshold?: number;
  /** 动画持续时间 */
  duration?: number;
...
export function useTouch(wrapperElement: HTMLElement, options: SwipeCellOptions = {}) {
  const {
    threshold = 0.5,
    duration = 300,
    disabled = false
  ...
  function getElementWidths() {
    const leftElement = wrapperElement.querySelector('.m-swipe-cell__left') as HTMLElement;
    const rightElement = wrapperElement.querySelector('.m-swipe-cell__right') as HTMLElement;
  ...
  // 获取当前状态
  function getState(): SwipeCellState {
    return { ...state };
  }
